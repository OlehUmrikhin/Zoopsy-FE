...
import { MdReport, MdPeople, MdArticle, MdNotifications } from 'react-icons/md';
...
const NAV_ITEMS: NavItem[] = [
  { to: '/moderator/complaints', label: 'Скарги', icon: MdReport },
  { to: '/moderator/users', label: 'Користувачі', icon: MdPeople },
  { to: '/moderator/content-moderation', label: 'Модерація', icon: MdArticle },
];
...
export function ModeratorLayout() {
  ...
  if (role !== 'moderator' && role !== 'admin') return <Navigate to="/bookings" />;
      ...
      <aside className="w-[256px] flex-shrink-0 flex flex-col bg-zoopsy-bg py-6">
        ...
        <nav className="flex flex-col gap-1 flex-1">
          {NAV_ITEMS.map(({ to, label, icon: Icon }) => {
            const isActive = pathname.startsWith(to + '/') || pathname === to;
            return (
              <Link
                key={to}
                to={to}
