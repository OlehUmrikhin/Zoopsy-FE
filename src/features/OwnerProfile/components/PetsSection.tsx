...
function PetRow({ index, control, register, onRemove }: PetRowProps) {
  ...
  return (
    <div className="bg-zoopsy-bg rounded-2xl p-4 flex items-center gap-4">
      ...
      <div className={`flex-1 grid gap-3 ${isDog ? 'grid-cols-[230px_1fr_1fr_1fr]' : 'grid-cols-[120px_1fr_1fr]'}`}>
        <Controller
    ...
    <div className="bg-white rounded-2xl p-6">
      ...
      <div className="flex flex-col gap-4">
        {fields.map((field, index) => (
          <PetRow key={field.id} index={index} control={control} register={register} onRemove={() => handleRemove(index)} />
        ))}
      </div>
    </div>
  )
